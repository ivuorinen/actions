...
RULES_DIR="validate-inputs/rules"
...
main() {
  ...
  # Check all YAML files in rules directory
  while IFS= read -r -d '' file; do
    if [[ -f "$file" && "$file" =~ \.(yml|yaml)$ ]]; then
      ((files_checked++))
      if ! check_file_manually_edited "$file"; then
        exit_code=1
      fi
    fi
  done < <(find "$RULES_DIR" -type f \( -name "*.yml" -o -name "*.yaml" \) -print0)
  ...
  if [[ $exit_code -eq 0 ]]; then
    ...
    echo ""
    echo -e "${YELLOW}📖 See validate-inputs/rules/README.md for more information${NC}"
  fi
