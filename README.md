...
| GitHub Actions       | [`release-monthly`][release-monthly], [`stale`][stale]                                                                                                             |
...
| YAML                 | [`ansible-lint-fix`][ansible-lint-fix], [`prettier-lint`][prettier-lint], [`sync-labels`][sync-labels]                                                             |
...
- **Self-Contained & Stdlib-Only**: each `<action>/validate.py` runs with `python3` — no `uv`, no third-party dependencies, and no shared action.
