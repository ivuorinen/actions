...
- **Validation**: Each action ships a self-contained `validate.py` (generated by `_validation/generate.py` from `_validation/kit.py` + `_validation/spec.py`).
  The harness validates inputs via `_tests/shared/validation_core.py --validate`, which delegates to that same kit.
...
Tests input validation by running the action's own self-contained `validate.py` (generated from the canonical `_validation/` kit):
...
```bash
# Don't manually test validation - use the validate_input_python helper
export INPUT_VERBOSE="true"
python3 my-action/validate.py
```
...
All input validation flows from the canonical validation kit in `_validation/`:

- `_validation/kit.py` defines `kit.CHECKS` — the single source of truth, one function per check type (token, version, file path, boolean, Docker, CodeQL, ...)
- `_validation/spec.py` maps each action's inputs to their check types
- `_validation/generate.py` emits a self-contained `<action>/validate.py` per action (pure stdlib)
- The ShellSpec harness validates via `_tests/shared/validation_core.py --validate`, which delegates to the same kit
- Injection, path-traversal, and token-format checks are enforced by the kit
...
- [Validation Kit Documentation](../_validation/README.md)
