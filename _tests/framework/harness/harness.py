...
'''
    ...
    # External service / network CLIs that must be mocked in hermetic tests.
    # GitHub-hosted runners install some of these (notably gh) into /usr/bin, so a
    # PATH that keeps the standard system dirs cannot exclude them. Each is shadowed
    # by a not-found stub in bin_dir (first on PATH) unless the test mocks it, so an
    # unmocked external command fails deterministically instead of reaching the real
    # binary on the runner — regardless of where that binary is installed.
    BLOCKED_COMMANDS = (
        "gh",
        "curl",
        "wget",
...
# generated by harness MockRegistry: unmocked external command blocked for hermeticity
printf '%s: not found (unmocked external command; mock it with mock_command)\\n' '{cmd}' >&2
exit 127
"""
    ...
    def materialize(session_dir: Path) -> Path:
        ...
        # Shadow unmocked external CLIs with a not-found stub so they fail
        # deterministically regardless of where they are installed on the runner.
        for cmd in MockRegistry.BLOCKED_COMMANDS:
            if cmd in commands:
                continue  # explicitly mocked -> real mock stub written above
            stub = bin_dir / cmd
            stub.write_text(MockRegistry.NOT_FOUND_TEMPLATE.format(cmd=cmd), encoding="utf-8")
            stub.chmod(stub.stat().st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)
...
def _execute_step(
    ...
    # bin_dir is first so mock stubs and the not-found stubs for unmocked external
    # CLIs (see MockRegistry.BLOCKED_COMMANDS) shadow anything below. Standard
    # coreutils dirs stay available so the shell can find sh/printf/grep/awk etc.
    # We can't exclude external CLIs by omitting dirs here — runners ship e.g. gh
    # in /usr/bin alongside coreutils — so hermeticity comes from the bin_dir stubs.
