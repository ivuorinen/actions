...
python3 '{dispatcher}' '{cmd}' '{mocks_json}' "$@"
"""
...
'''
    ...
    def materialize(session_dir: Path) -> Path:
        ...
        mocks = json.loads(mocks_path.read_text())

        bin_dir = session_dir / "bin"
        bin_dir.mkdir(exist_ok=True)

        dispatcher = session_dir / "_dispatcher.py"
        dispatcher.write_text(MockRegistry.DISPATCHER_TEMPLATE)
...
def _parse_github_output(github_output: Path) -> dict[str, str]:
    """Parse key=value lines from $GITHUB_OUTPUT for use in later if: expressions."""
    out: dict[str, str] = {}
