...
Context "when file path is under _tests/ (intentional bash usage)"
It "exempts _tests/run-tests.sh from bash-ism checks"
input=$(make_input "_tests/run-tests.sh" '[[ \$x == y ]]')
When run bash -c "printf '%s' '$input' | sh '$HOOK'"
The status should be success
The output should equal ""
End
It "exempts _tests/framework/ helpers"
input=$(make_input "_tests/framework/setup.sh" "local x=1")
When run bash -c "printf '%s' '$input' | sh '$HOOK'"
The status should be success
The output should equal ""
End
It "exempts _tests/unit/ specs"
input=$(make_input "_tests/unit/foo/spec.sh" "arr=(a b c)")
When run bash -c "printf '%s' '$input' | sh '$HOOK'"
The status should be success
The output should equal ""
End
